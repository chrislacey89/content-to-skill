...
describe('library_index.ts', () => {
  ...
  it('skips incomplete book.json (missing required fields)', () => {
    ...
    fs.cpSync(path.join(FIXTURES_DIR, 'books', 'test-book'), path.join(booksDir, 'test-book'), { recursive: true });
    fs.cpSync(path.join(FIXTURES_DIR, 'books', 'incomplete-book'), path.join(booksDir, 'incomplete-book'), { recursive: true });
